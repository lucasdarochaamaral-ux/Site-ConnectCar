...
const WhatsAppFloat: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const hasTypingShown = sessionStorage.getItem('typingShown') === 'true';
  ...
  const handleStartChat = () => {
    ...
    if (typeof window !== 'undefined' && (window as any).dataLayer) {
      (window as any).dataLayer.push({ event: 'clique_whatsapp_flutuante' });
    }
  };
  ...
  return (
    <>
      ...
      <div
        onClick={togglePopup}
        className="fixed bottom-5 left-5 rounded-full shadow-lg flex items-center justify-center cursor-pointer z-50 hover:scale-105 transition-transform"
        style={{
      ...
      >
        ...
        <Phone size={24} color="white" className="z-10" />
      </div>

      {/* Popup */}
      {isPopupOpen && (
        <div
          className="fixed bottom-24 left-5 w-75 bg-white rounded-xl shadow-2xl z-50 overflow-hidden"
          style={{
            width: '300px',
        ...
        >
          {/* Header */}
          <div 
            className="flex items-center justify-center relative p-3 text-white font-bold"
          ...
          >
            ...
            <button 
              onClick={closePopup}
              className="absolute right-3 bg-transparent border-none text-white text-xl cursor-pointer hover:opacity-80"
            >
              ×
            </button>
          </div>
          ...
          >
            ...
            {showTyping && (
              <div 
                className="mb-2.5 p-2.5 rounded-lg text-sm max-w-4/5 relative"
                style={{
          ...
          >
            <a
              ...
              className="text-white font-bold py-2 px-4 rounded no-underline hover:opacity-90 transition-opacity"
              style={{ backgroundColor: '#25D366' }}
            >
              Iniciar Conversa
            </a>
