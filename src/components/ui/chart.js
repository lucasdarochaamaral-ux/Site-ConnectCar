...
var ChartTooltipContent = React.forwardRef(function (_a, ref) {
    ...
    return (<div ref={ref} className={(0, utils_1.cn)("grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-zinc-200 border-zinc-200/50 bg-white px-2.5 py-1.5 text-xs shadow-xl dark:border-zinc-800 dark:border-zinc-800/50 dark:bg-zinc-950", className)}>
        ...
        <div className="grid gap-1.5">
          {payload.map(function (item, index) {
            ...
            return (<div key={item.dataKey} className={(0, utils_1.cn)("flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5 [&>svg]:text-zinc-500 dark:[&>svg]:text-zinc-400", indicator === "dot" && "items-center")}>
                {formatter && (item === null || item === void 0 ? void 0 : item.value) !== undefined && item.name ? (formatter(item.value, item.name, item, index, item.payload)) : (<>
                    ...
                    <div className={(0, utils_1.cn)("flex flex-1 justify-between leading-none", nestLabel ? "items-end" : "items-center")}>
                      ...
                      {item.value && (<span className="font-mono font-medium tabular-nums text-zinc-950 dark:text-zinc-50">
...
var ChartLegendContent = React.forwardRef(function (_a, ref) {
    ...
    return (<div ref={ref} className={(0, utils_1.cn)("flex items-center justify-center gap-4", verticalAlign === "top" ? "pb-3" : "pt-3", className)}>
        {payload.map(function (item) {
            ...
            var itemConfig = getPayloadConfigFromPayload(config, item, key);
            return (<div key={item.value} className={(0, utils_1.cn)("flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-zinc-500 dark:[&>svg]:text-zinc-400")}>
