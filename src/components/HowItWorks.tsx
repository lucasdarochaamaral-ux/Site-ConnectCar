...
import { Package, Wrench, MapPin } from 'lucide-react';
...
const HowItWorks: React.FC = () => {
  ...
  const steps = [
    {
      icon: <Package size={48} className="text-orange-600" />,
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            setTimeout(() => setVisibleSteps(prev => [true, prev[1], prev[2]]), 300);
            setTimeout(() => setVisibleSteps(prev => [prev[0], true, prev[2]]), 800);
            setTimeout(() => setVisibleSteps(prev => [prev[0], prev[1], true]), 1300);
  ...
  return (
    <section id="como-funciona" className="py-16 bg-white dark:bg-gray-900 transition-colors duration-300" ref={sectionRef}>
        ...
        <div className="relative max-w-5xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-8 md:gap-4 relative">
            ...
            {steps.map((step, index) => (
              <div 
                key={index} 
                className={`flex-1 flex flex-col items-center text-center max-w-xs mx-auto relative z-10 group cursor-pointer transition-all duration-700 ${
                  visibleSteps[index] 
              ...
              >
                ...
                <div className={`absolute -top-2 -left-2 w-8 h-8 bg-orange-600 text-white rounded-full flex items-center justify-center text-sm font-bold z-20 shadow-lg transition-all duration-500 ${
                  visibleSteps[index] 
                    ? 'scale-100 shadow-2xl animate-pulse-orange' 
                    : 'scale-0'
                } group-hover:scale-125 group-hover:bg-orange-500 group-hover:shadow-2xl`}>
                  {index + 1}
                </div>
                
                {/* Container do ícone sem fundo bege */}
                <div className={`mb-6 p-6 rounded-full transition-all duration-700 shadow-md ${
                  visibleSteps[index] 
                    ? 'scale-100 shadow-xl' 
                    : 'scale-75'
                  ...
                  <div className={`transition-all duration-500 ${
                    visibleSteps[index] 
                      ...
                      : 'scale-75 brightness-75'
                  } group-hover:scale-110 group-hover:brightness-125`}>
                    {step.icon}
                  </div>
                </div>
                
                <h3 className={`text-xl font-semibold text-gray-800 dark:text-white mb-3 transition-all duration-500 ${
                  visibleSteps[index] ? 'text-gray-800 dark:text-white' : 'text-gray-400 dark:text-gray-400'
                } group-hover:text-orange-600`}>
                  {step.title}
                </h3>
                <p className={`text-gray-600 dark:text-white dark:font-semibold leading-relaxed transition-all duration-500 ${
                  visibleSteps[index] ? 'text-gray-600 dark:text-white dark:font-semibold' : 'text-gray-400 dark:text-gray-400'
          ...
          <div className="md:hidden flex flex-col items-center mt-8">
            {[0, 1].map((index) => (
              <div key={index} className="w-1 h-16 bg-gray-200 rounded-full relative overflow-hidden my-4">
                <div className={`w-full h-full bg-gradient-to-b from-orange-400 to-orange-600 rounded-full transition-all duration-1000 ${
                  visibleSteps[index] ? 'opacity-100' : 'opacity-30'
                }`}></div>
              </div>
            ))}
          </div>
